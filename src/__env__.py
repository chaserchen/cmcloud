...
LOGGING_LEVEL_CONFIG = objectify({
    ...
    'job_worker': 'info', # just control how pyres do the logging, our code still controlled by LOGGING_LEVEL_CONFIG.cmcloud
    'cmcloud_postgresql': {
...
def cmcloud_periodic_job_scheduler_program(config):
    return periodic_job_scheduler_program(LOGGING_LEVEL_CONFIG.cmcloud, application_config=cmcloud_config(config))

def cmcloud_job_worker_program(worker_name, queue_names, config, run_as=None, count=1, timeout=180):
    return job_worker_program(worker_name, LOGGING_LEVEL_CONFIG.job_worker, LOGGING_LEVEL_CONFIG.cmcloud, config.queue_host, config.queue_port,
        queue_names=queue_names, application_config=cmcloud_config(config), run_as=run_as, count=count, timeout=timeout)


def routines_job_worker_program(config, count=1):
    return cmcloud_job_worker_program('routines',
        [
...
def transactional_email_worker_program(config, count=1):
    return cmcloud_job_worker_program('transactional_email', ['send_transactional_email'], config, count=count)
...
def cmcloud_postgresql_program(config, more_config=None):
    more_config = more_config or {}
    return log_rotated_postgresql_program('cmcloud', config.cmcloud_postgresql_version, config.cmcloud_postgresql_host, config.cmcloud_postgresql_port,
        ...
        log_min_duration_statement=LOGGING_LEVEL_CONFIG.cmcloud_postgresql.log_min_duration_statement, **more_config)
...
def teacher_website_programs(config):
    return website_programs('teacher', LOGGING_LEVEL_CONFIG.cmcloud, application_config=cmcloud_config(config), start_port=config.teacher_website_start_port,
...
def cmcloud_config(config):
    cmcloud_config_ = objectify({
        'queue_client': {
    ...
    for purpose in REDIS_CLIENTS:
        cmcloud_config_['{}_redis_client'.format(purpose)] = {
            'host': config['{}_redis_host'.format(purpose)],
            'port': config['{}_redis_port'.format(purpose)]
        }
    teacher_website_authority = config.teacher_website_domain if config.teacher_website_domain_port in (80, 443) else '{}:{}'.format(
        config.teacher_website_domain, config.teacher_website_domain_port)
    for purpose in TEACHER_WEBSITE_BUCKETS:
    ...
    for purpose in WEBSITES:
        cmcloud_config_['{}_website'.format(purpose)] = {
            ...
            'master_template_directory': VEIL_HOME / 'src' / 'cmcloud' / 'website' / purpose,
            'prevents_xsrf': True,
